...
public class GetConfigDialogTask implements Callable<JDialog> {
  ...
  public GetConfigDialogTask(boolean isGateway) {
  ...
  public JDialog call() throws IBControllerException, InterruptedException {
    ...
    if (isGateway) {
       ...
       * the menu items become responsive: any attempt to access the Configure >
       * Settings menu item (even after it has been enabled) results in an exception
       * being logged by TWS.
       * 
       * It's not obvious how long we need to wait before the menu becomes responsive.
       * However the splash frame that appears in front of the gateway main window
       * during initialisation disappears when everything is ready, and its close can
       * be detected as a frame entitled 'Starting application...' and a Closed event.
       * 
  ...
  public void setConfigDialog(JDialog configDialog) {
    lock.lock();
    try {
