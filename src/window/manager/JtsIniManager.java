...
public class JtsIniManager {
  ...
  public static void initialise(String jtsIniPath) {
    jtsIniFilePath = jtsIniPath;
  }

  /*
   * when TWS starts, there must exist a jts.ini file in the TWS settings
   * directory containing at least the following minimum contents:
   *
   * [Logon] s3store=true
   *
   ...
   * contains at least both sets of lines shown above, with the exception that if
   * it already exists and contains s3store=false then this setting will not be
   * altered.
   */
  public static void ensureValidJtsIniFile() {
    loadIniFile();
