...
public class DefaultLoginManager extends LoginManager {
  ...
  public DefaultLoginManager() {
    /*
     * don't actually get the credentials yet because the settings provider might be
     * changed
     */
    fromSettings = true;
  ...
  public DefaultLoginManager(String[] args, Encryptor encryptor) {
  ...
  /**
   * IBAPI username - can either be supplied from the .ini file or as args[1] NB:
   * if IBAPI username is supplied in args[1], then the password must be in
   * args[2]. If IBAPI username is supplied in .ini, then the password must also
   * be in .ini.
   */
  private volatile String IBAPIUserName;

  /**
   * unencrypted IBAPI password - can either be supplied from the .ini file or as
   * args[2]
   */
  private volatile String IBAPIPassword;

  /**
   * FIX username - can either be supplied from the .ini file or as args[1] NB: if
   * username is supplied in args[1], then the password must be in args[2], and
   * the IBAPI username and password may be in args[3] and args[4]. If username is
   * supplied in .ini, then the password must also be in .ini.
   */
  private volatile String FIXUserName;

  /**
   * unencrypted FIX password - can either be supplied from the .ini file or as
   * args[2]
   */
  ...
  private String getFIXPasswordFromSettings() {
  ...
  private String getTWSPasswordFromSettings() {
