...
public class Utils {
  ...
  /**
   * Performs a click on the menu item at the specified path, waiting if necessary
   * for the menu item to become enabled.
   * 
   * If there is more than one menu bar within the specified container, they are
   * searched (in hierarchical containment order) until one is found that contains
   * the specified menu item.
   * 
   * Note that this method may block the calling thread if the required menu item
   ...
   * @return true if the menu item was successfully clicked; false if the menu
   *         item could not be found
   * @throws IllegalStateException
   *           the method has been called on the Swing event dispatch thread
   */
  public static boolean invokeMenuItem(final Container container, final String[] path) throws IllegalStateException {
   ...
   */
  public static void logRawToConsole(String msg) {
   ...
   * 
   * Note that this method swallows the InterruptedException that may result from
   * a call to sleep().
   * 
   * @param millis
   ...
   * @param path
   *          the path to the required configuration section in the Global
   *          Configuration dialog
   * @return true if the specified section can be found; otherwise false
   ...
   */
  public static boolean selectConfigSection(final JDialog configDialog, final String[] path) throws IBControllerException, IllegalStateException {
  ...
  public static void showTradesLogWindow() {
    MyCachedThreadPool.getInstance().execute(new Runnable() {
      @Override
      public void run() {
        invokeMenuItem(MainWindowManager.mainWindowManager().getMainWindow(), new String[] { "Account", "Trade Log" });
      }
    });
  }

  public static void sendConsoleOutputToTwsLog(boolean value) {
